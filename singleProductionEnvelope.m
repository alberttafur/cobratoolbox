function [x, ymin, ymax, x_KO, ymin_KO, ymax_KO ] = singleProductionEnvelope(model, deletions, product, biomassRxn, varargin)
...
%
% OUTPUTS;
%    x:               x axis for the curves - 'Wild-type'
%    ymin:            y results for the first curve - 'Minimum Wild-type'
%    ymax:            y results for the second curve - 'Maximum Wild-type'
%    x_KO:            x axis for the curves - 'Mutant'
%    ymin_KO:         y results for the first curve - 'Minimum Mutant'
%    ymax_KO:         y results for the second curve - 'Maximum Mutant'
%
...
x_KO = linspace(0,max,nPts)';
ymin_KO = zeros(nPts,1);
ymax_KO = zeros(nPts,1);

for i = 1:nPts
    modelY = changeRxnBounds(modelKO, biomassRxn, x_KO(i), 'b');
    modelY = changeObjective(modelY, product);
    fmin = optimizeCbModel(modelY, 'min');
    fmax = optimizeCbModel(modelY, 'max');
    ymin_KO(i) = fmin.f;
...
% plot
hold on
plot(x_KO, ymin_KO, 'r', x_KO, ymax_KO, 'm', 'LineWidth', 2);
...
if savePlot
    ...
    if ~isempty(outputFolder)
        ...
        mkdir([folder filesep 'optKnockresults']);
        cd([folder filesep 'optKnockresults']);
    end
